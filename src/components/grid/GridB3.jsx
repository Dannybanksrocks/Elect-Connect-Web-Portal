import { reduce, voteResultsbyCounty_District } from "../../functions";
...
const GridB3 = ({ districtAccess }) => {
  const { votesDetails } = useVotes();
  const row = votesDetails
    ? votesDetails.filter((item) => item.districtName === districtAccess)
        .length > 0 &&
      voteResultsbyCounty_District(votesDetails, districtAccess)
    : null;
  const votesValid = votesDetails ? reduce(votesDetails, "votesValid") : null;
